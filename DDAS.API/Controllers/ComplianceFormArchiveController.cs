...
namespace DDAS.API.Controllers
{
    ...
    public class ComplianceFormArchiveController : ApiController
    {
            ...
            try
            {
                using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
                {
                    ...
                    foreach (MultipartFileData file in provider.FileData)
                    {
                        ...
                        var excelInput =
                                _compFormArchiveSerive.ReadDataFromExcelFile(FilePathWithGUID);
                        ...
                        var forms = _compFormArchiveSerive.ReadUploadedFileData(
                            excelInput,
                            userName,
                            FilePathWithGUID,
                            UploadedFileName);
                        ...
                        foreach (ComplianceFormArchive form in forms)
                        {
                            ...
                            complianceForms.Add(
                                _compFormArchiveSerive.ScanUpdateComplianceForm(
                                    form));
        ...
        [HttpPost]
        //public async Task<HttpResponseMessage> UploadAttachments(ComplianceFormArchive form)
        public async Task<HttpResponseMessage> UploadAttachments(string SessionId)
        {
            ...
            try
            {
                using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
                {
                    ...
                    //_compFormArchiveSerive.AddAttachmentsToFindings(form);
                    return Request.CreateResponse(HttpStatusCode.OK);
                }
            }
            catch (Exception e)
        ...
        public IHttpActionResult GetMyActivePrincipalInvestigators()
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ...
                var UserName = User.Identity.GetUserName();
                return Ok(
                    _compFormArchiveSerive.getPrincipalInvestigators(UserName, true));
            }
        }
        ...
        public IHttpActionResult GetMyReviewPendingPrincipalInvestigators()
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                var UserName = User.Identity.GetUserName();
                //return Ok(
                //    _compFormArchiveSerive.getPrincipalInvestigators(UserName, true, false));

                return Ok(
                    _compFormArchiveSerive.getPrincipalInvestigators(UserName, ReviewStatusEnum.ReviewInProgress));
            }
        }
        ...
        public IHttpActionResult GetMyClosedPrincipalInvestigators()
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                var UserName = User.Identity.GetUserName();
                return Ok(
                    _compFormArchiveSerive.getPrincipalInvestigators(UserName, false));
            }
        }
        ...
        public IHttpActionResult GetMyReviewCompletedPrincipalInvestigators()
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                var UserName = User.Identity.GetUserName();
                return Ok(
                    _compFormArchiveSerive.getPrincipalInvestigators(UserName, true, true));
            }
        }
            ...
            string SiteDataId, SiteEnum SiteEnum, string FullName)
        {
            try
            {
                using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
                {
                    ...
                    return Ok(
                        _compFormArchiveSerive.GetSingleComponentMatchedRecords(
           ...
           string userName)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.GetUserFullName(userName));
            }
        }
        ...
        public IHttpActionResult GetComplianceFormFilterResults(ComplianceFormFilter CompFormFilter)
        {
            try
            {
                using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
                {
                    var xyz = _SearchService.GetComplianceFormsFromFilters(CompFormFilter);
                    var result = _compFormArchiveSerive.GetComplianceFormsFromFilters(CompFormFilter);
                    return Ok(xyz);
                }
            }
            catch (Exception Ex)
            ...
            ComplianceFormFilter CompFormFilter)
        {
            try
            {
                using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
                {
                    ...
                    return Ok(
                        _compFormArchiveSerive.GetClosedComplianceFormsFromFilters(
        ...
        public IHttpActionResult GetUnAssignedComplianceForms()
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.GetUnAssignedComplianceForms());
            }
        }
        ...
        public IHttpActionResult GetComplianceForm(string formId = "")  //returns previously generated form or empty form  
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ...
                if (formId == null)
                {
                    return Ok(_compFormArchiveSerive.GetNewComplianceForm(UserName, "Manual"));
                }
                else
                {
                    Guid? gFormId = Guid.Parse(formId);
                    var compForm = _UOW.ComplianceFormArchiveRepository .FindById(gFormId);
                    if (compForm == null)
                    {
                        return NotFound();
                    }
                    else
                    {
                            ...
                            compForm.AssignedTo.ToLower() == User.Identity.GetUserName().ToLower())
                        {
                            ...
                            //_UOW.ComplianceFormArchiveRepository .UpdateCollection(compForm);
                        }
                        _UOW.ComplianceFormArchiveRepository .UpdateCollection(compForm);
                    }
                    return Ok(compForm);
        ...
        [HttpPost]
        public IHttpActionResult UpdateQCEditComplianceForm(ComplianceFormArchive form)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.UpdateQC(form));
            }
        }

        [Route("SaveComplianceForm")]
        [HttpPost]
        public IHttpActionResult UpdateComplianceForm(ComplianceFormArchive form)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.UpdateComplianceForm(form));
            }
        }

        [Route("ScanSaveComplianceForm")]
        [HttpPost]
        public IHttpActionResult ScanUpdateComplianceForm(ComplianceFormArchive form)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ...
                var result = _compFormArchiveSerive.ScanUpdateComplianceForm(form);
                return Ok(result);
            }
        }
        #endregion
            ...
            string ComplianceFormId)
        {
            try
            {
                ...
                _compFormArchiveSerive.UpdateAssignedTo(RecId, AssignedBy, AssignedFrom, AssignedTo);
        ...
        public IHttpActionResult AssignComplianceFormsTo(AssignComplianceFormsTo AssignComplianceFormsTo)
        {
            try
            {
                ...
                {
                    var AssignedBy = User.Identity.GetUserName();
                    _compFormArchiveSerive.UpdateAssignedTo(AssignedBy, AssignComplianceFormsTo);
                    return Ok(true);
                }
            }
            catch (Exception ex)
        ...
        public IHttpActionResult ClearAssginedTo(string ComplianceFormId, string AssignedFrom)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                var AssignedBy = User.Identity.GetUserName();
                var RecId = Guid.Parse(ComplianceFormId);
                _compFormArchiveSerive.UpdateAssignedTo(RecId, AssignedBy, AssignedFrom, "");
        ...
        [HttpPost]
        public IHttpActionResult UpdateCompFormGeneralNInvestigatorsNOptionalSites(ComplianceFormArchive form)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.UpdateCompFormGeneralNInvestigatorsNOptionalSites(form));
            }
        }
        ...
        public IHttpActionResult UpdateFindings(UpdateFindigs updateFindings)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.UpdateFindings(updateFindings));
                //return Ok( _UOW.ComplianceFormArchiveRepository .UpdateFindings(updateFindings));
            }
        }
        ...
        public IHttpActionResult UpdateInstituteFindings(UpdateInstituteFindings FindingsModel)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                return Ok(_compFormArchiveSerive.UpdateInstituteFindings(FindingsModel));
            }
        }
        ...
        public HttpResponseMessage GenerateComplianceForm(string ComplianceFormId)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ...
                else
                {
                    ...
                    //var FilePath = _compFormArchiveSerive.GenerateComplianceForm(
                    //    RecId, 
                    //    writer, 
                    //    ".docx", 
                    //    out FileName);
                    ...
                    var memoryStream = _compFormArchiveSerive.GenerateComplianceForm(
                        RecId,
                        writer,
                        ".docx",
                        out FileName);
        ...
        public HttpResponseMessage DownloadForm(string ComplianceFormId = null)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ...
                else
                {
                    ...
                    var form = _compFormArchiveSerive.GenerateComplianceForm(Guid.Parse(ComplianceFormId));
        ...
        public IHttpActionResult CloseComplianceForm(Guid ComplianceFormId)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ComplianceFormArchive form = _UOW.ComplianceFormArchiveRepository .FindById(ComplianceFormId);
        ...
        public IHttpActionResult DeleteComplianceForm(string ComplianceFormId)
        {
            ...
            {
                Guid? RecId = Guid.Parse(ComplianceFormId);
                _UOW.ComplianceFormArchiveRepository .DropComplianceForm(RecId);
                return Ok(true);
            }
        }
        ...
        public IHttpActionResult MoveToCompletedICSF(string ComplianceFormId)
        {
            using (new TimeMeasurementBlock(Logger, _logMode, CurrentUser(), GetCallerName()))
            {
                ...
                var Form = _UOW.ComplianceFormArchiveRepository .FindById(Id);
        ...
        public IHttpActionResult ExportToiSprint(string ComplianceFormId)
        {
            ...
            {
                var RecId = Guid.Parse(ComplianceFormId);
                var Form = _UOW.ComplianceFormArchiveRepository .FindById(RecId);
                var ExportResponse = _compFormArchiveSerive.ExportDataToIsprint(Form);
