...
namespace DDAS.Data.Mongo.Repositories
{
        ...
        IComplianceFormArchiveRepository
    {
        ...
        //Not used:
        public ComplianceFormArchive FindComplianceFormIdByNameToSearch(string NameToSearch)
        {
            var filter = Builders<ComplianceFormArchive>.Filter.Eq("NameToSearch", NameToSearch);
            var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
            var entity = collection.Find(filter).FirstOrDefault();
            return entity;
        }

        public List<ComplianceFormArchive> FindActiveComplianceForms(bool value)
        {
            var Filter = Builders<ComplianceFormArchive>.Filter.Eq("Active", value);
            var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
        ...
        public List<ComplianceFormArchive> FindComplianceForms(string AssignedTo, ReviewStatusEnum ReviewStatus)
        {
            var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
        ...
        public List<ComplianceFormArchive> FindComplianceForms(string AssignedTo)
        {
            var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
            var entity = collection.Find(
                x => x.AssignedTo == AssignedTo
        ...
        public List<ComplianceFormArchive> FindComplianceForms( ReviewStatusEnum ReviewStatus)
        {
            var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
            var entity = collection.Find(
        ...
        public List<ComplianceFormArchive> FindComplianceForms(ComplianceFormFilter CompFormFilter)
        {
            ...
            {
                //var filter = Builders<ComplianceFormArchive>.Filter
        ...
        public List<ComplianceFormArchive> FindQCComplianceForms(DateTime? ReviewAssignedOn, DateTime? ReviewCompletedOn)
        {
            var builder = Builders<ComplianceFormArchive>.Filter;
            var filter = builder.Empty;
        ...
        public Task UpdateCollection(ComplianceFormArchive form)
        {
            return _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name).
        ...
        public Boolean UpdateComplianceForm(ComplianceFormArchive form)
        {
            
            var currentRowVersion = form.RowVersion;
            form.RowVersion = NewRowVersion();

            var result =  _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name).
        ...
        //{
        //    var filter = Builders<ComplianceFormArchive>.Filter.Eq("_id", id);

        //    var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
        //    var update = Builders<ComplianceFormArchive>.Update
        ...
        //Not used??
        public bool UpdateComplianceForm(Guid id, ComplianceFormArchive form)
        {
            var filter = Builders<ComplianceFormArchive>.Filter.Eq("_id", id);
        ...
        public bool UpdateInvestigator(Guid formId, InvestigatorSearched Investigator)
        {
            ...
            var filter = Builders<ComplianceFormArchive>.Filter.Where(x => x.RecId == formId);
            //var inv = new List<InvestigatorSearched>();
            //inv.Add(Investigator);
            var update = Builders<ComplianceFormArchive>.Update.AddToSet(x => x.InvestigatorDetails, Investigator);
            //var update1 = Builders<ComplianceFormArchive>.Update.Set(x => x.InvestigatorDetails, inv);
        ...
        public bool UpdateFindings(UpdateFindigs updateFindings)
        {
            ...
            //var filter = Builders<ComplianceFormArchive>.Filter.Where(x => x.RecId == updateFindings.FormId);
            //var update = Builders<ComplianceFormArchive>.Update.PullFilter(x => x.Findings,
            ...
            var filter1 = Builders<ComplianceFormArchive>.Filter.Where(x => x.RecId == updateFindings.FormId);
            var update1 = Builders<ComplianceFormArchive>.Update.AddToSetEach(x => x.Findings, updateFindings.Findings);
        ...
        public bool AddFindings(Guid formId, List<Finding> findings)
        {
            ...
            var collection = _db.GetCollection<ComplianceFormArchive>(typeof(ComplianceForm).Name);
            var filter = Builders<ComplianceFormArchive>.Filter.Where(x => x.RecId == formId);
            var update = Builders<ComplianceFormArchive>.Update.AddToSetEach(x => x.Findings, findings);
             var result = collection.UpdateOneAsync(filter, update).Result;
            if (result.IsAcknowledged && result.ModifiedCount == 1)
            {
                return true;
        ...
        {
            //var filter1 = Builders<ComplianceFormArchive>.Filter.Eq("_id", form.Findings.
